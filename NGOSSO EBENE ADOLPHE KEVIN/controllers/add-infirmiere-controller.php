...
if (isset($_POST['submit_infirmiere'])) {
    ...
    if (check_user_email($data['email'], $db) === 0) {
        if (check_user_pseudo($data['pseudo'], $db) === 0) {
            ...
            if ($error === true) {
                ...
                require 'views/add-infirmiere.php';
            } else {
                add_infirmiere($_SESSION['hopital_id'],$data, $db);
                echo "<script language='javascript'>alert(' l'Infirmiere a bien été ajouté !!')</script>";
                die('<meta http-equiv="refresh" content="1 ; URL=index.php?page=views/personnel">');
