    ...
    if (check_user_email($data['email'], $db) === 0) {
        if (check_user_pseudo($data['pseudo'], $db) === 0) {
            ...
            if ($error === true) {
                ...
                require 'views/add-medecin.php';
            } else {
                add_medecin($_SESSION['hopital_id'],$data, $db);
                echo "<script language='javascript'>alert(' le Medecin a bien été ajouté !!')</script>";
